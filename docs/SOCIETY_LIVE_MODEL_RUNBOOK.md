...
| `abandon:<candidate-id>` | closes ONE stranded candidate through `POST /v1/society/candidates/<id>/abandon` — the operator path, never SQL. Same discipline as `refute`: the driver holds a database connection and could set `status='abandoned'` in one statement, which would leave no audit row and release no escrow. It asserts the title, `created_at` and correlation are untouched, reads the implementation task's status and the caller wallet's `reserved_credits` back **out of** the database to prove the escrow was released exactly once through the ordinary path, repeats the call to prove no second release and no second audit row, and checks the event names an *operator*. `PHASE5_ABANDON_REASON` is required | `abandon.before/after/events`, checks `A01–A09` |
