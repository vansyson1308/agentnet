...
| `tests/society/*` (36 files) | RUNS IN CI | PostgreSQL | whole package skips with a reason if Postgres is unreachable; CI provides it. Includes `test_authz_registry.py`, `test_authz_payment.py`, `test_money_concurrency.py` (real-Postgres races), `test_websocket_auth.py`, `test_sandbox_paths.py`, `test_config_validation.py`, `test_canary.py` (deterministic adapters only — never a live model). Phase 3 self-development gates (also run as a named CI step): `test_repo_intel.py` (path-safe bounded reads), `test_risk_and_meta_change.py` (trusted-base tiers, policy self-edit attack), `test_promotion.py` (controller state machine, faults, races, crash recovery, branch==main/force-push refusal), `test_fitness.py` (hard gates, corpus, reward hacking, criteria-change attack), `test_memory_provenance.py` (provenance, decay, poisoning), `test_telemetry_scout_budget.py` (producers, evidence, budgets, anti-busywork), `test_deepseek_contract.py` (fake-transport output-format negotiation), `test_deepseek_live_compat.py` (Phase 4.1 / ADR-0007: DeepSeek-shaped fixtures A–K reproducing the thinking-consumed-budget defect, capability profiles never send DeepSeek-only fields on `generic`, fail-fast reasoning config, the runtime's non-thinking structured request, no chain of thought kept, reasoning tokens counted once), `test_secret_boundary.py` (no credential in model context), `test_live_prompt_contract.py` (Phase 5: nested payload models documented in the prompt, trusted engineering conventions reach only engineering roles and match the acceptance test, HTTP canary importable without the ORM), `test_deployment_and_jarvis.py`, `test_e2e_self_development.py` (source-code candidate → shadow PR → fitness → rollback recommendation; Phase 3.1 no-legacy-side-effect block). Phase 3.1 gates: `test_single_control_plane.py` (the real general worker loop never touches Society proposals or writes a file backlog; one signal → one workstream; nothing starts the legacy synthetic agents), `test_github_credentials_and_push.py` (credential providers, App JWT/token lifecycle with a fake endpoint, 401/429/5xx/timeout/concurrency, argv/URL/config-safe `git push` with a recording runner and a real push to the local Basic-auth git HTTP harness `git_http_harness.py`) |
| `tests/test_config_parity.py` | RUNS IN CI | — | `SocietySettings` ⇄ `.env.example` ⇄ `docker-compose.staging.yml` ⇄ `docs/DEPLOYMENT_ARCHITECTURE.md` contract; safe staging defaults; no secret compose literals; no stale proxy claim |
| `tests/test_railway_adaptation.py` | RUNS IN CI | `git`, `bash` | Phase 4 (ADR-0006): `SKIP_DB_BOOTSTRAP=true` execs without touching the database while the default path still bootstraps; the registry image ships the society bootstrap script; the volume bootstrap script against a local bare origin (first clone, reuse, alignment to `RAILWAY_GIT_COMMIT_SHA`, dirty reset, candidate worktrees kept, stale metadata pruned, credentialed/non-HTTPS URLs and unknown commits refused, never pushes or forces); `.railway/railway.ts` invariants (staging-only, no secret literals, one volume, Society OFF, migration owner, no custom domain) and name parity with `.env.example`; the dashboard client honours `REGISTRY_URL`, renders the registry's bare-array public listing and renders with the registry down (link integrity is the public-surface gates' job, docs/PUBLIC_SURFACE_CONTRACT.md); `/readyz` discloses nothing |
| `tests/test_proxy_headers.py` | RUNS IN CI | — | Phase 4 (ADR-0006 D5): `X-Real-IP` client-address middleware (opt-in `TRUST_X_REAL_IP`), `X-Forwarded-For` never read, non-IP values ignored, WebSocket scheme mapping, scope not mutated; demonstrates that uvicorn with `FORWARDED_ALLOW_IPS=*` honours the client-written leftmost `X-Forwarded-For`; registry wiring order |
| `tests/test_railway_validator.py` | RUNS IN CI | — | Phase 4 (ADR-0006 D12): the in-environment staging validator — spoof-test analysis (forged headers must not earn a fresh bucket), password derivation from the validator secret only, script-output summarisation keeps verdict lines only, no secret value in any printed line. Phase 5: `VALIDATOR_EXPECT_RUNTIME` defaults to `off`; the live-society driver `deploy/railway/phase5_live.py` — plan grammar, secret-shape scrubbing and chunked JSON lines, gates only narrow, bounded idempotent fund key, public-surface private-key walker, chain checks K01–K10 (fake provider / DEAD run / executed HIGH intent / skipped acceptance / off-list files / duplicate candidate all fail), escrow-ledger invariants E01–E03, never-print contract |
| `tests/society/fixtures/code_repo/*` | TEST FIXTURE | — | isolated application with one planted defect, copied into a throw-away git repo by the source-code E2E and `demo --story code`; `collect_ignore_glob` keeps it out of the repo suite |
...
| `examples/demo_autonomous_society.py` (`--story docs` and `--story code`) | RUNS IN CI (step) | PostgreSQL | deterministic Society end-to-end (documentation candidate; real source-code candidate with shadow promotion + offline fitness) |
