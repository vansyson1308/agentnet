 ...
 *   - no public surface beyond exactly two custom domains, created on Railway
 *     before the DNS cutover and routed only once the owner publishes their DNS
 *     records (docs/PRODUCTION_CUTOVER.md): api.agentnet.io.vn -> prod-registry
 *     :8000 and dashboard.agentnet.io.vn -> prod-dashboard :8080. No Railway
 *     service domain, no TCP proxy, and never a domain on payment, worker,
 *     Postgres or Redis. An undeclared custom domain would be DELETED on apply,
 *     which is why both are declared here.
...
export default defineRailway((ctx) => {
  ...
  /**
   * The one browser origin the public API accepts: the dashboard's public
   * https origin. Today's dashboard calls the registry server-side over private
   * DNS (REGISTRY_URL below) and needs no CORS at all; this admits exactly the
   * origin any future in-browser call would come from, and nothing else --
   * never "*", never a Railway-generated domain.
   */
  const PUBLIC_DASHBOARD_ORIGIN = "https://dashboard.agentnet.io.vn";
  ...
  const registry = service("prod-registry", {
    ...
    env: {
      ...
      CORS_ALLOWED_ORIGINS: PUBLIC_DASHBOARD_ORIGIN,
  ...
  const dashboard = service("prod-dashboard", {
    ...
    domains: [{ domain: "dashboard.agentnet.io.vn", port: 8080 }],
    healthcheck: "/healthz",
    healthcheckTimeout: 120,
    env: {
      ENVIRONMENT: "production",
