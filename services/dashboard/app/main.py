...
@app.route("/metaverse/world")
def metaverse_world():
    """Metaverse World view – placeholder for immersive experience."""
    return render_template("metaverse_world.html")

@app.route("/marketplace")
def marketplace_page():
    """Public marketplace – browse agents without login."""
    agents = []
    try:
        agents = api_client.fetch_agents()
    except Exception as e:
        flash(f"Could not load marketplace: {e}", "warning")
...
@app.route("/directory")
def directory_page():
    """List all agents registered by the user."""
    ...
    try:
        ...
        flash(f"Error fetching agents: {e}", "danger")
...
def wallet_page():
    ...
    try:
        ...
        flash(f"Error fetching wallet data: {e}", "danger")
...
def create_offer_page():
    if "access_token" not in session:
        ...
        return redirect(url_for('login_page'))
    if request.method == "POST":
        # Simple offer creation placeholder
        flash("Offer creation is not yet implemented.", "info")
        return redirect(url_for('metaverse_page'))
    return render_template("create_offer.html")

# ============================================================
# API-LIKE ENDPOINTS (for AJAX calls, mostly read-only)
# ============================================================

@app.route("/api/agents/public")
def api_public_agents():
    """JSON endpoint for marketplace browsing (no auth)."""
    try:
        agents = api_client.fetch_agents()
...
# ============================================================
# STATIC FILES – serve via Flask in dev only (Caddy handles in prod)
# ============================================================
...
# ============================================================
# METAVERSE‑SPECIFIC PLACEHOLDER ENDPOINTS
# ============================================================

@app.route("/metaverse/v1/status")
def metaverse_status():
    """Simple status endpoint for metaverse internal checks."""
    return jsonify({
        "version": "1.0.0",
        "environment": _ENV,
...
@app.route("/metaverse/v1/agents")
def metaverse_agents():
    """JSON feed of agents for metaverse UI (authenticated)."""
    if "access_token" not in session:
        return jsonify({"error": "unauthorized"}), 401
    try:
        agents = api_client.get_my_agents()
