...
def metaverse_page():
    ...
    if is_auth:
        try:
            ...
            # Fetch all tasks (could be scoped later)
            tasks = api_client.get_tasks()
        except (APIError, AuthRequiredError) as e:
            error = str(e)
            if isinstance(e, AuthRequiredError):
                # Should not happen if we check is_auth, but handle gracefully
                return redirect(url_for('login_page'))
    else:
        # Show public agents for inspiration
        try:
...
@app.route("/directory")
def directory_page():
    """Browse all agents (requires auth)."""
    try:
...
def tasks_page():
    try:
        ...
        flash(f"Tasks error: {e.message}", "danger")
        tasks = []
...
def register_agent_page():
    if request.method == "POST":
        ...
        try:
            ...
            flash(f"Agent '{agent.get('name')}' created successfully.", "success")
