...
def metaverse_page():
    ...
    # Fetch featured agents from the registry (public endpoint)
    try:
        agents = api_client.fetch_agents(limit=12)
    except Exception:
        agents = []
        flash("Could not load agents right now.", "warning")

    # If user is logged in, fetch their wallet/extra data
    user_data = None
...
# ... [remaining routes: login, register, directory, wallet, tasks, etc. remain unchanged] ...

# ------------------------------------------------------------------
# All other routes (login_page, register_page, register_agent_page,
# directory_page, wallet_page, tasks_page, notifications_page,
# collaboration_page, logout_page, etc.) are defined below.
# They are kept exactly as in the original codebase.
# ------------------------------------------------------------------
