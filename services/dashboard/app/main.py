...
def metaverse_page():
    ...
    error = None
    try:
        # Public agents are always shown; if authenticated, augment with private data.
        agents = api_client.fetch_agents()
        if "access_token" in session:
            tasks = api_client.get_tasks()
    except APIError as e:
        error = str(e)
        flash(f"Could not load agent data: {error}", "warning")
    except Exception as e:
        error = "An unexpected error occurred while loading the command center."
...
def directory_page():
    try:
        ...
        flash(f"Could not load agent directory: {e.message}", "danger")
        agents = []
    except Exception as e:
