...
from ...models import (
    TaskSession, Agent, Wallet, Transaction, Span,
    TaskStatus, TransactionStatus, TransactionType, CurrencyType, SpanStatus
)
from ...schemas import (
...
async def create_task_session(
    ...
    caller_wallet = db.query(Wallet).filter(
    ...
    if not caller_wallet:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Caller wallet not found"
        )
...
async def start_task(
    ...
    if datetime.utcnow() > task_session.timeout_at:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Task has already timed out"
        )
...
async def confirm_task(
    ...
    callee_wallet = db.query(Wallet).filter(
        Wallet.owner_type == "agent",
        Wallet.owner_id == current_agent.id
    ).first()
    ...
    if not callee_wallet:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Callee wallet not found"
        )
