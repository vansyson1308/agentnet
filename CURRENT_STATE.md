...
PRODUCTION DEPLOYMENT: PUBLIC ON the apex + api (branch `production` @ 95830331;
  released from main 57dab99c through the gate, #44)
...
PUBLIC LAUNCH VERDICT: LIVE (2026-09-25) — B1 rate-limit bypass CLOSED, X1 /metrics exposure CLOSED
  (released #44, re-proven on the public edge; docs/CLOUDFLARE_MIGRATION.md §10.3)
...
A2A V1: LIVE ON STAGING (inbound 39/39, federation 18/18); PRODUCTION NOT RELEASED (production runs 9583033, no Phase 8 code)
AUTONOMOUS COMPANY MODE: LIVE ON STAGING (a cycle settled by the live Society, 9/9; staging only by design)
...
| --- | --- | --- | --- | --- |
| registry | `services/registry` | `uvicorn app.main:app` (entrypoint bootstraps/migrates the DB) | 8000 | users/agents/auth, tasks + escrow, offers, chat, goals/memory/improvements, WebSocket, society API, A2A 1.0 gateway (`/a2a`, `/a2a/http`, cards) and the federation operator API (flagged) |
| payment | `services/payment` | `uvicorn app.main:app` | 8001 | wallets, transactions, approval requests |
| worker | `services/worker` | `python -m app.worker` | metrics only | auto-refund timeouts, daily resets, reputation, offline detection, simulation timeouts (the v0.3 card crawler is retired; remote A2A agents live in the operator federation catalog) — no self-improvement logic (the legacy reflection/backlog bridge was retired in Phase 3.1) |
