...
- Federation needs `A2A_REFERENCE_CARD_URL`, pointing at a disposable service built from `scripts/a2a/reference/` (the official helloworld peer). After the proof, mode `federation_cleanup` blocks the peer's catalog entries and revokes its open connections (X01/X02), and the service is deleted.
- Production runs it from `prod-validator` with `A2A_PROOF_IDENTITY=public`: an owner-verified account made through the public signup, never a database read (docs/PRODUCTION_RUNBOOK.md, "Re-running the A2A proof in production").
- Company mode waits for the Society to settle the cycle. A cycle settles only once it is 30 minutes old (`company.SETTLE_AFTER`) and every run on its correlation has finished, so `A2A_PROOF_CYCLE_TIMEOUT` defaults to 45 minutes. Set `A2A_PROOF_CYCLE_ID` (an id or id prefix) to follow a cycle that is already running instead of opening another one. It then lists the cycle's role runs with their model provider (every run that reached cognition must be the live provider) and the intents it produced.
